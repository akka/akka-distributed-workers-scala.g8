...
object Master {
  ...
  case class RegisterWorker(workerId: String, replyTo: ActorRef[Worker.Command])
      extends Command
              ...
              case Cleanup =>
                ...
                workers.foreach {
                  ...
                  case (workerId, WorkerState(_, Idle, lastHeardFrom))
                    ...
                    ctx.log.info(
                      "Too long since heard from worker {}, pruning",
                      workerId
                    )
                    workers -= workerId
  ...
  case class WorkerState(ref: ActorRef[Worker.Command],
                         status: WorkerStatus,
